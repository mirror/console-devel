...
//				}
					...
					&dwBytesRead))
				{
					...
					if (peb_upp.CurrentDirectoryPath.Length > 0) {
						// Yes, try to read CurrentDirectoryPath
						pwszBuffer = (WCHAR *)HeapAlloc(hHeap,
							HEAP_ZERO_MEMORY,
							peb_upp.CurrentDirectoryPath.Length);

						// If memory was allocated, continue
						if (pwszBuffer)
						{
							if (ReadProcessMemory(hProcess,
								peb_upp.CurrentDirectoryPath.Buffer,
								pwszBuffer,
								peb_upp.CurrentDirectoryPath.Length,
								&dwBytesRead))
							{
								// if commandline is larger than our variable, truncate
								if (peb_upp.CurrentDirectoryPath.Length >= sizeof(spi.szCurrentDirectoryPath)) 
									dwBufferSize = sizeof(spi.szCurrentDirectoryPath) - sizeof(TCHAR);
								else
									dwBufferSize = peb_upp.CurrentDirectoryPath.Length;
								...
								// there is no conversion if application is
								// compiled for Unicode
								StringCbCopyN(spi.szCurrentDirectoryPath, sizeof(spi.szCmdLine),
									pwszBuffer, dwBufferSize);
#else
								// Since core NT functions operate in Unicode
								// we must convert to Ansi since our application
